...
func opWrite(in writeInput) (map[string]any, error) {
	...
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		return failure(fmt.Sprintf("create kubeconfig dir: %v", err)), nil
	}
	if err := clientcmd.WriteToFile(*cfg, path); err != nil {
		return failure(fmt.Sprintf("write kubeconfig %q: %v", path, err)), nil
	}
...
func opRemove(in removeInput) (map[string]any, error) {
	...
	if err != nil {
		...
		return failure(fmt.Sprintf("load kubeconfig %q: %v", path, err)), nil
	}
...
func opCurrentServer(in currentServerInput) (map[string]any, error) {
	...
	cfg, err := rules.Load()
	if err != nil {
		return failure(fmt.Sprintf("load kubeconfig: %v", err)), nil
	}
