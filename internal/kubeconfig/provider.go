...
func inputSchema() *jsonschema.Schema {
	return sdkhelper.ObjectSchema(
		...
		map[string]*jsonschema.Schema{
			...
			"ca_data":              sdkhelper.StringProp("PEM-encoded cluster CA bundle; preferred over insecure_skip_tls"),
			"interactive_mode":     sdkhelper.StringProp("Exec plugin interactive mode", sdkhelper.WithEnum("Never", "IfAvailable", "Always")),
			"install_hint":         sdkhelper.StringProp("Message kubectl shows when the exec command is missing from PATH"),
			"provide_cluster_info": sdkhelper.BoolProp("Pass cluster details to the exec plugin via KUBERNETES_EXEC_INFO"),
