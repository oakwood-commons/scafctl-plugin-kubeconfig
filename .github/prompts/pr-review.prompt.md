...
3. **Coverage check**: Assess patch coverage against the **70% target** (see `codecov.yml`):
   ...
   - If Codecov has not reported yet (common on a fresh repo), fall back to local coverage: run `task test` and inspect the per-package/per-file coverage it records
